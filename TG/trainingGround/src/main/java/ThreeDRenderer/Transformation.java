...
public class Transformation {
	...
	public Matrix4f getModelViewMatrix(GameItem gameItem, Matrix4f viewMatrix) {
		...
		modelViewMatrix.set(viewMatrix).translate(gameItem.getPosition()).rotateX((float) Math.toRadians(-rotation.x))
	...
	public Matrix4f buildModelViewMatrix(GameItem gameItem, Matrix4f viewMatrix2) {
		Vector3f rotation = gameItem.getRotation();
		modelMatrix.identity().translate(gameItem.getPosition()).rotateX((float) Math.toRadians(-rotation.x))
				.rotateY((float) Math.toRadians(-rotation.y)).rotateZ((float) Math.toRadians(-rotation.z))
				.scale(gameItem.getScale());
